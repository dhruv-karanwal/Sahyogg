      ...
      body = Center(
        child: Padding(
          ...
          return StreamBuilder<QuerySnapshot>(
            stream: SafeZoneService().publicSafeZonesStream,
            builder: (context, snap) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  data['id'] = docs[index].id; // Optional: keep ID

                  // Handle coordinate: stored as latitude/longitude fields
                  final double lat = (data['latitude'] as num?)?.toDouble() ?? 0.0;
                  final double lng = (data['longitude'] as num?)?.toDouble() ?? 0.0;
                  
                  // Construct zone map for UI: needs 'coordinate' list for MapScreen compatibility
                  final zoneForMap = {
                    ...data,
                    'coordinate': [lat, lng],
                    // ensure fields exist
                    ...
                    'operationalStatus': data['status'] ?? 'Unknown', // Map 'status' to 'operationalStatus'
                  };
                          ...
                          builder: (_) => SafeZoneMapScreen(
                            zones: docs.map((d) {
                              ...
                              return {
                                ...dData,
                                'coordinate': [
                                  (dData['latitude'] as num?)?.toDouble() ?? 0.0,
                                  (dData['longitude'] as num?)?.toDouble() ?? 0.0
                                ],
                                'operationalStatus': dData['status'],
                              };
                            }).toList(),
                            userLocation: LatLng(
                              _userPos!.latitude,
