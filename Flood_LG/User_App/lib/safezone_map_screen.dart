...
class _SafeZoneMapScreenState extends State<SafeZoneMapScreen> {
  ...
  LatLng _zoneLatLng(int idx) {
    try {
      ...
      if (c == null || c is! List || c.isEmpty) return const LatLng(0, 0);
      
      final lat = (c[0] is String) ? double.parse(c[0]) : (c[0] as num).toDouble();
      final lng = (c[1] is String) ? double.parse(c[1]) : (c[1] as num).toDouble();
      
      return LatLng(lat, lng);
    } catch (e) {
      print('Error parsing coordinate for zone $idx: $e');
