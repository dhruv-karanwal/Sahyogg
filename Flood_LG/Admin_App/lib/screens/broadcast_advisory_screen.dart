...
import '../controllers/advisory_service.dart';
...
class _BroadcastAdvisoryScreenState extends State<BroadcastAdvisoryScreen> {
  ...
  Future<void> _sendAdvisory() async {
    ...
    try {
      await AdvisoryService().sendAdvisory(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Broadcasted & Saved to History: "${_limitText(_lastPostedMessage!, 30)}..."'),
  ...
  Future<void> _clearAdvisory() async {
    ...
    try {
      await AdvisoryService().clearAdvisory();
